"""
Train PhoBERT for XSS detection.

Features used:
    - PhoBERT tokenized input (max_len=256)
...
# ---------------------------------------------------------------------------
# Model: PhoBERT [CLS] + handcrafted features -> classifier
# ---------------------------------------------------------------------------
class PhoBertWithFeatures(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, extra):
        ...
        fused = torch.cat([cls, extra], dim=1)
...
def train():
    ...
    model     = PhoBertWithFeatures().to(device)
    ...
    print(f'\nTraining PhoBERT + Handcrafted ({EPOCHS} epochs)...')
    ...
    print_report(y_test, y_pred, 'PhoBERT + Handcrafted')

    cm = confusion_matrix(y_test, y_pred)
    fig, ax = plt.subplots(figsize=(5, 4))
    plot_confusion_matrix(cm, 'PhoBERT (Test Set)', 'Oranges', ax)
    plt.tight_layout()
    plt.savefig(os.path.join(os.path.dirname(MODEL_DIR), 'cm_phobert.png'), dpi=150)
    plt.show()
